...
public class ExamMain {
	//1. 상속방법 : class 클래스(자식) extends 클래스 (부모)
	//2. 자식 클래스는 부모 클래스로 부터 메소드와 필드를 물려받아 사용한다.
	//3. 부모 클래스는 자식 클래스에서 정의한 메소드나 필드를 사용하지 못한다.
	//(자식 = 자신 + 부모/ 부모=자신)
	//4. extends로 클래스의 상속은 한가지만 가능하다(단일상속)
	...
	public static void main(String[] args) {
		...
		sc.sum(1, 2);
		sc.minus(1, 2);
		//sub클래스에 모든 기능이 있기 때문에 각각을 인스턴스화 하는것은 효율적이지 않음
