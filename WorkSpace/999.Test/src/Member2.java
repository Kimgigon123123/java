...
public class Member2 {
	...
	public Member2(int field) {//메소드의 중갈호(블럭킹)가 끝나면 내부에 있는 모든 내용은 메모리에서 사라짐
		this.field = field;
	}
	...
	//                 (인스턴스화 과정이 끝나고 나서는 전체 멤버가 사용 가능함, 외부에서 private 제외하고)
	// 생성자(Constuctor) 메소드: 클래스가 메모리에 올라갈때 사용할 생성식 == 클래스와 이름이 대소문자가 정확히 같음
	
	public static void main(String[] args) {
		
		Member2 mm = new Member2(10);//인스턴스화 과정 : new 'Member();'생성자 메소드
